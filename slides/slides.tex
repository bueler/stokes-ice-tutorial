...
\begin{itemize}
\item \emph{purpose:} additional useful and robust features
    \begin{itemize}
    \item[$\circ$] rescale the equations  \hfill \emph{for better conditioning}
    \item[$\circ$] vertical grid sequencing  \hfill \emph{for better initial iterates}
    \item[$\circ$] $100\eps$ on coarse meshes in sequencing  \hfill \emph{for better initial iterates}
    \item[$\circ$] generate stress tensor $\tau$  \hfill \emph{as diagnostic}
    \item[$\circ$] generate effective viscosity $\nu_\eps$  \hfill \emph{as diagnostic}
    \end{itemize}
\item \emph{source files:} \texttt{solve.py} \hfill \alert{$\gets$ inspect this!}
\item \emph{generated files:} \texttt{dome\_0.vtu}, \texttt{dome.pvd}
\end{itemize}
...
\begin{Verbatim}
$ cd stage5/
$ mpiexec -n P ./solve.py -refine 0 \
    -s_snes_atol 1.0e-2 -s_snes_monitor
$ paraview dome.pvd
$ mpiexec -n P ./solve.py -bumpy -baserefine 5 \
