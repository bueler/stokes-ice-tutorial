...
\item conservation principles apply to ice sheets; each one is a PDE
    ...
    \end{itemize}
\item Stokes model combines momentum conservation with incompressibility
    \begin{itemize}
    \item[$\circ$] incompressibility is one aspect of mass conservation
    \end{itemize}
\item \alert{Stokes model determines velocity and pressure from given geometry}
...
\bigskip
    \begin{itemize}
    \item[$\circ$] no attempt \emph{today} to model geometry changes using surface balance
    \end{itemize}
\end{itemize}
\end{frame}
...
\item so we solve numerically using the finite element (FE) method
    \begin{itemize}
    \item[$\circ$] as implemented in Firedrake
    \item[$\circ$] \alert{\dots the whole point is that we \emph{don't} need to know details}
    \end{itemize}
...
\bigskip
\item we need to know about the weak form!
\end{itemize}
\end{frame}
...
\item in Firedrake language (= \emph{Unified Form Language}) it looks like:
\begin{Verbatim}[fontsize=\small]
f = Constant((0.0, 0.0, - rho * g))
F = ( inner(nueps(D(u)), D(v)) \
...
\bigskip
\item I use an alias \texttt{drakeme} for this
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \emph{purpose:} try PETSc solvers which scale with increasing $N$
\item \emph{source files:} \texttt{solve.py} \hfill \alert{$\gets$ inspect this!}
\item \emph{generated files:} \texttt{dome\_0.vtu}, \texttt{dome.pvd}
\end{itemize}
