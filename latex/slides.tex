...
\date[June 2026]{version June 2026  \\ \phantom{foo} \\ \phantom{foo} \\
...
\item conservation principles apply to ice sheets:
    \begin{itemize}
    \item[$\circ$] mass \hfill \cmark
    \item[$\circ$] momentum \hfill \cmark
    \item[$\circ$] energy \hfill \xmark
    \end{itemize}
...
\item one needs to know the first step:\, write a weak form
\end{itemize}
\end{frame}
...
\medskip
\item the problem statement solved by the FE method:
...
\item \emph{purpose:} solve same problem using an extruded quadrilateral mesh
    \begin{itemize}
    \item[$\circ$] simplifies the toolchain; Gmsh no longer needed
    \end{itemize}
...
$ python3 solve.py -mx 320 -mz 2 -refine 2  # high resolution
$ paraview dome.pvd
\end{Verbatim}
%$
...
\hfill \includegraphics[width=0.4\textwidth]{figs/stage5.png} \hspace{10mm}
\end{frame}
