...
if args.dim == 2:
    sbase.dat.data[:] = profile2d(xbase)
    bbase.dat.data[:] = bed2d(xbase, args.b0)
else:
    sbase.dat.data[:] = profile3d(xbase, ybase)
...
for j in range(args.refine + 1):
    ...
    if args.dim == 2:
        ...
        XZ = Function(Vcoord).interpolate(as_vector([x, (s - b) * z + b]))
